...
export default class ProductUpdatePage {
  ...
  psizeSelect: ElementFinder = element(by.css('select#product-psize'));
  ...
  async setPsizeSelect(psize) {
    await this.psizeSelect.sendKeys(psize);
  }

  async getPsizeSelect() {
    return this.psizeSelect.element(by.css('option:checked')).getText();
  }

  async psizeSelectLastOption() {
    await this.psizeSelect.all(by.tagName('option')).last().click();
  }
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.psizeSelectLastOption();
    await waitUntilDisplayed(this.saveButton);
